...
final readonly class DepositEligibilityPolicy
{
    ...
    /**
     * Ensures user is eligible to make a deposit
     * 
    ...
    private function assertUserVerified($user): void
    {
        if (!$user->isVerified()) {
            throw new PaymentNotAllowedException(
                'Account verification required. Please complete KYC to make deposits.'
            );
        }
    }
    ...
    private function assertOnboardingComplete($user): void
    {
        if (!$user->hasCompletedOnboarding()) {
            throw new PaymentNotAllowedException(
                'Please complete account setup before making deposits.'
            );
        }
    }

    /**
     * Check if user can deposit without throwing exception
     */
